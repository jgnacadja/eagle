<template>
  <article class="rounded-md border border-rule bg-paper p-md shadow-sm">
    ...
    <p class="mt-xs text-small text-ink-subtle">{{ departments }}</p>
    <div class="mt-md flex flex-wrap gap-sm">
      <span
        v-for="tag in tags"
        :key="tag"
        class="inline-flex items-center gap-sm rounded-full px-md py-xs text-meta font-medium"
        :class="tagClasses(tag)"
      >
        <span v-if="!tag.startsWith('⚠')" class="h-xs w-xs rounded-full" :class="dotClass(tag)" />
        {{ tag }}
      </span>
    </div>
  </article>
