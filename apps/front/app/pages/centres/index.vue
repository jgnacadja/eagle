  ...
  <div class="bg-surface">
    <section class="mx-auto max-w-container px-gutter-mobile md:px-gutter py-section">
      ...
      <div class="mt-2xl max-w-prose">
        <p class="text-overline font-bold text-accent-text">LE RÉSEAU LEARN UP</p>
        <h1 class="mt-sm font-display text-h1 font-extrabold text-ink md:text-hero">
          Réseau de centres
        </h1>
      ...
      <!-- Filters -->
      <div class="mt-2xl flex flex-col gap-md sm:flex-row sm:items-center sm:justify-between">
      ...
      <div class="mt-md flex items-center justify-between gap-sm">
        <p class="text-small text-ink-muted">
          <strong class="text-ink">{{ resultsText }}</strong>
        </p>

        <button
          type="button"
          class="lg:hidden inline-flex h-control shrink-0 items-center gap-sm rounded-full border border-outline bg-paper px-md text-small font-semibold text-ink transition hover:bg-surface"
          @click="openMobileMap"
        >
          <svg
            class="h-4 w-4"
      ...
      <!-- Content -->
      <div class="mt-xl grid gap-xl lg:grid-cols-[1fr_2fr]">
        <!-- List -->
        ...
        <div
          v-else
          class="rounded-md border-2 border-dashed border-rule bg-paper p-2xl text-center"
        >
          <h2 class="font-sans text-h4 font-semibold text-ink">
            Aucun centre n'est implanté dans ce département pour le moment.
          </h2>
          <p class="mt-sm text-small text-ink-muted max-w-prose mx-auto">
          ...
          <div class="mt-xl flex flex-wrap items-center justify-center gap-md">
            <NuxtLink
              to="/"
              class="rounded-full bg-ink px-md py-sm text-small font-semibold text-paper transition hover:bg-primary-dark"
            >
              Demander une formation
            </NuxtLink>
            <NuxtLink
              to="/"
              class="rounded-full border border-outline bg-paper px-md py-sm text-small font-semibold text-ink transition hover:bg-surface"
            >
    ...
    <ClientOnly>
      <Teleport to="body">
        <div
          v-if="isMobileMapOpen"
          class="fixed inset-0 z-40 flex flex-col bg-paper"
        >
          <div class="flex items-center justify-between gap-sm border-b border-rule px-gutter-mobile py-md">
...
const activeCenter = computed(() => filteredCenters.value.find((c) => c.id === activeCenterId.value))
