...
export const bubbleVariants = cva(
  'gap-xs data-[align=end]:self-end max-w-[85%] data-[variant=ghost]:max-w-full group-data-[align=end]/message:self-end group/bubble relative flex w-fit min-w-0 flex-col',
  {
    variants: {
      variant: {
        default:
        ...
        surface: '*:data-[slot=bubble-content]:bg-surface *:data-[slot=bubble-content]:text-ink',
        tinted:
          '*:data-[slot=bubble-content]:bg-primary-soft *:data-[slot=bubble-content]:text-foreground [&>[data-slot=bubble-content]:is(button,a):hover]:bg-primary-muted',
        outline:
...
export const bubbleReactionsVariants = cva(
  'rounded-full ring-3 ring-card bg-surface-alt shrink-0 gap-xs px-sm py-xs has-[button]:p-0 text-meta absolute z-10 flex w-fit items-center justify-center',
  {
    variants: {
      side: {
