...
- Helmet, `@nestjs/throttler` + stockage Redis (`@nest-lab/throttler-storage-redis`) à mettre en place pour le catalogue.
- Limites cibles :
  - lecture publique `/formations*` : 100 requêtes/min par IP
...
- Modèles attendus pour le sprint 2 :
  - `Formation` : `digiforma_id` unique, slug, titre, description, durée, prix, CPF, code CPF, certification, catégorie/famille, blocs (JSON), image, statut, champs SEO, raw Digiforma, timestamps.
...
- Mapping `Program` → `Formation` ; en cas de doute, garder le payload brut dans `raw`.
