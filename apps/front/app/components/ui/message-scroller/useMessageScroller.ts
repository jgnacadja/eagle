...
export type MessageScrollerScrollBehavior = 'auto' | 'instant' | 'smooth'
...
function measureMessageVisibility(
  element: HTMLElement,
  {
    anchorLine,
  ...
  const rect = isAnchor || noIntersectionObserver ? element.getBoundingClientRect() : null
  const visible = noIntersectionObserver
    ? Boolean(rect && rect.bottom > anchorLine && rect.top < viewportBottom)
    : visibleMessageIds.has(messageId)
  return {
    anchor: Boolean(isAnchor && rect && rect.top <= anchorLine + SCROLL_EPSILON),
    messageId,
    visible
  }
}
...
function computeVisibility({
  ...
  const viewportRect = viewport.getBoundingClientRect()
  const options = {
    anchorLine: viewportRect.top + scrollMargin + scrollPreviousItemPeek,
    noIntersectionObserver: typeof IntersectionObserver === 'undefined',
  ...
  for (const child of getMessageChildren(content, spacer)) {
    const message = measureMessageVisibility(child, options)
    if (!message) continue
    if (message.visible) visible.push(message.messageId)
    if (message.anchor) currentAnchorId = message.messageId
  }
...
export interface MessageScrollerContext {
  ...
  handleResize: () => void
  scrollToEnd: (options?: { behavior?: MessageScrollerScrollBehavior }) => boolean
  scrollToMessage: (messageId: string, options?: MessageScrollerScrollOptions) => boolean
  scrollToStart: (options?: { behavior?: MessageScrollerScrollBehavior }) => boolean
...
function createEngine(props: MessageScrollerProviderProps) {
  ...
  function scrollTo(
    ...
    }: { behavior?: MessageScrollerScrollBehavior; autoscrolling?: boolean } = {}
  ) {
    if (!viewport) return
  ...
  function scrollToStart({
    behavior = 'auto'
  }: { behavior?: MessageScrollerScrollBehavior } = {}): boolean {
  ...
  function applyInitialContentChange(children: HTMLElement[]) {
    if (
  ...
  function applyNewAnchor(children: HTMLElement[], previousCount: number): boolean {
    if (children.length <= previousCount) return false
  ...
  function applyUnhandledAnchor(children: HTMLElement[], previousCount: number): boolean {
    if (children.length !== previousCount) return false
  ...
  function applyContentChange(
    ...
    if (previousCount === 0) {
      applyInitialContentChange(children)
      return
    }
    ...
    if (applyNewAnchor(children, previousCount) || applyUnhandledAnchor(children, previousCount))
      return
    settleContentChange()
  }
