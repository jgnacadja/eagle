    ...
    <div class="mx-auto px-gutter-mobile py-section md:px-gutter">
      ...
      <div class="grid grid-cols-1 items-start gap-lg lg:grid-cols-3">
        ...
        <aside class="order-first space-y-lg sm:sticky sm:top-lg lg:order-last">
          ...
          <Card v-if="besoinParam" v-reveal class="border-accent/40 bg-accent-soft/40 p-lg">
...
const onSubmit = handleSubmit(async (v) => {
  const ok = await submitLead('demande', {
    ...
    // — précédé du libellé pour le distinguer des précisions saisies.
    precisions:
      [besoinParam.value ? `Besoin exprimé : « ${besoinParam.value} »` : '', v.precisions ?? '']
        .filter(Boolean)
        .join('\n\n') || undefined,
